 ...
 * until we run out of candies.  The last person will receive all of our remaining candies (not necessarily one more than
 * the previous gift).
 *
...
public class S1103 {
    public static int[] distributeCandies(int candies, int num_people) {
        ...
        double t = (Math.sqrt(1+8l*candies) - 1)/2;
        int it = (int)t;
        int rest = candies - (it+1) * it/2;
        for(int i=1;i<=it;i++) {
            ret[(i-1)%num_people] += i;
        }
        ret[it%num_people] += rest;
        return ret;
    }

    public static void main(String[] args) {
        int[] b = distributeCandies(1000000000, 1000);
        for(int a:b) {
            System.out.print(" " + a);
        }
    }
